...
- **来源可追溯**：每条新闻单独发卡片，展示频道发布时间（北京时间），区分事实与模型解读；卡片不展示事件时间字段、不附带原文外链。摘要超过80字或超过2行时，在核心速览后追加「事件详情」。
...
采集全天继续。夜间重要候选存入 SQLite，超过实时新闻30分钟年龄仍可进入晨报；晨报只回顾当夜窗口，标明北京时间（不附原文外链），不能冒充即时快讯。09:00后不补发迟到晨报。模型失败可在窗口内重试；发送结果不明不自动重发，重启也不会重复发送。
