...
- **分层去重**：频道/消息ID、规范化正文指纹、24小时投递记录，以及最近20条投递摘要供模型比较新增事实。新闻卡片在飞书发送前再做一次**确定性近重复校验**（同组 24h 内 `title+summary` 与已投递摘要的实体/数字重合；`$6.06` 与 `$6` 视为同一量级）。同事件且未带实质更新（`is_update` + `update_reason` 中的新数字或至少两个新实体）则跳过发送、标 `near_duplicate`、不记为已投递。同一事件的**新角度仍可推送**，但发送前会去掉与近期投递重合的要点，卡片只保留增量、不复述上一张已推送的前提；洗完后若没有新要点才跳过。不额外调用模型。`wechat_photo` 不走此守卫。
...
- **改写后的重复**：正文指纹只处理规范化后相同的正文。新闻路径在发送前用模板剥离后的中文二元组 + 数字桶做近重复拦截（Jaccard ≥0.45 且至少 1 个非套话重合，或 Jaccard ≥0.30 且 ≥4 个实体重合）；仅改标题、补加州/创纪录、`$6`→`$6.06` 会被挡住。模型仍可能把同事件选进 digest，但飞书不会发出第二张。真正不同的宏观事件（如柴油 vs 美联储）不受影响。同一主体的后续新角度（新承诺/新数字）可以再发，卡片正文不得把上一张已推送的结论再讲一遍。`wechat_photo` / 分组隔离不变。
