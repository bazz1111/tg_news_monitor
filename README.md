...
采集全天继续。夜间重要候选存入 SQLite，超过实时新闻30分钟年龄仍可进入晨报；晨报只回顾当夜窗口，标明事件时间与原文链接，不能冒充即时快讯。09:00后不补发迟到晨报。模型失败可在窗口内重试；发送结果不明不自动重发，重启也不会重复发送。
...
晨报默认从最多20条候选中精选，按重要性和发布时间排序；模型合并同事件、保留输入中的最新进展。它不是完整夜间新闻档案，语义去重与事实更新时间仍依赖模型。夜间摘要缓存写入时清理三天前内容。
