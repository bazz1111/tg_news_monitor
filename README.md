...
语义去重和时间提取依赖模型，不能保证百分之百准确。严格时效可能漏掉时间不明的消息；进程崩溃或投递不明也可能漏推。当前没有完整持久化发件箱、PDF/OCR或独立研报摘要通道。夜间熔断依赖关键词或高分，仍可能漏报或误报。
...
| 时段 | 本地时钟 | 评估 | 推送 |
|---|---|---|---|
| 白天 | 08:00–23:00 | 沿用 `DIGEST_*` / `HOTNESS_THRESHOLD` | 评分 ≥7 |
| 肩时段 | 23:00–01:00 | 至少16条，最小间隔10分钟 | 评分 ≥8，卡片间隔15秒 |
| 深夜 | 01:00–08:00 | 默认不因最长等待强行评估；紧急关键词或攒满候选才调用模型（间隔30分钟） | 仅评分 ≥9 或命中 `urgent()` 关键词；每夜最多5张卡片，计数存在 SQLite，08:00 重置 |
| 晨间冲刷 | 进入白天时一次 | 仍计入每日调用上限，不受最小间隔限制 | 放宽年龄至2小时，评分 ≥7，卡片间隔10秒 |

深夜未评估的积压留给约08:00的一次冲刷。采集轮询不受时段影响。
...
| `SHOULDER_HOURS` | `23:00-01:00` | `23:00-01:00` | 肩时段，可跨午夜；空=关闭 |
| `QUIET_HOURS` | `01:00-08:00` | `01:00-08:00` | 深夜窗口；空=关闭 |
...
| `QUIET_HOTNESS_THRESHOLD` | 9 | 9 | 深夜推送评分下限（关键词可绕过） |
| `QUIET_DIGEST_MIN_INTERVAL_SECONDS` | 1800 | 1800 | 深夜模型最小间隔，秒 |
| `QUIET_DIGEST_MIN_CANDIDATES` | 16 | 16 | 深夜非紧急评估门槛 |
| `QUIET_CARD_CAP` | 5 | 5 | 单个深夜窗口飞书卡片上限 |
| `MORNING_FLUSH_MAX_AGE_SECONDS` | 7200 | 7200 | 晨间冲刷最大年龄，秒 |
| `MORNING_FLUSH_HOTNESS_THRESHOLD` | 7 | 7 | 晨间冲刷推送评分下限 |
| `MORNING_FLUSH_CARD_INTERVAL_SECONDS` | 10 | 10 | 晨间冲刷卡片间隔，秒 |
...
建议从每分钟采集、模型最短3分钟、普通新闻最长等待10分钟开始，观察三天实际流量再调整。紧急消息约3–5分钟是正常负载目标，不是SLA；未识别为紧急的单条消息可能等10分钟，还需加上网络和模型耗时。23:00后门槛抬高，01:00–08:00默认几乎不推，08:00左右最多冲刷一次仍较新鲜的积压。
...
升级前停止旧实例并一致性备份数据库及配置，再更新代码、合并参数、重建镜像。新增 `digest_calls`、`delivery_claims` 和 `alert_schedule_state` 表在runner启动时自动创建。不要删库重置预算、去重和夜间卡片计数。

新投递历史从升级后记录，不自动迁移旧版已发送摘要；原消息ID去重保留。当前建议单实例运行。回滚旧代码后新保护不再生效。容器健康检查仅检查CLI能运行，不能证明采集、模型或推送正常。

## 常见情况

- **不推送**：检查抓取成功率、门槛、冷却、预算、年龄、评分及当前北京时间窗口，并非每条帖子都会推送。深夜默认几乎不推，除非高分或紧急关键词，且受每夜5张卡片限制。
